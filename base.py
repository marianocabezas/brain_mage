...
class Encoder(BaseModel):
    ...
    def forward(self, input_s):
        ...
        features, _ = torch.max(input_s, (2, 3, 4))
...
class DoubleResConv3dBlock(BaseConv3dBlock):
    def __init__(
            self, filters_in, filters_out,
            kernel=3, norm=None, activation=None
    ):
        ...
        self.conv1 = self.conv(filters_in, filters_in)
        self.conv2 = self.conv(filters_in, filters_out)
        ...
        self.end_seq1 = nn.Sequential(
            activation(filters_in),
            norm(filters_in)
        )
        self.end_seq2 = nn.Sequential(
            activation(filters_out),
            norm(filters_out)
        )
