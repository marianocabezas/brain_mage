...
def mutual_information_loss(fixed, moved, mask=None):
    if mask is None:
        hist_fixed, bins = torch.histogram(fixed, bins=256)
...
def compute_matrices(alpha, beta, gamma, T):
    ...
    cos_a = torch.cos(alpha / 2)
    cos_b = torch.cos(beta / 2)
    cos_g = torch.cos(gamma / 2)
    sin_a = torch.sin(alpha / 2)
    ...
    return affine_fwd, affine_bck


def halfway_rigid_registration(
    image_a, image_b, spacing_a, spacing_b, mask_a=None, mask_b=None,
    shape_target=None, spacing_target=None,
    scales=None, epochs=500, patience=100, init_lr=1e-3, loss_f=xcor_loss,
    device=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
    ...
    for s in scales:
        ...
        for e in range(epochs):
            ...
            optimizer.step()
        alpha, beta, gamma, T = best_params
        print('Epoch {:03d} [scale {:02d}]: {:8.4f}'.format(
            final_e + 1, s, final_fit
        ))
        best_fit = np.inf
        # lr = lr / 5
    affine_fwd, affine_bck = compute_matrices(alpha, beta, gamma, T)
    return affine_fwd, affine_bck, final_e, final_fit
'''
    ...
    for s in scales:
        ...
        for e in range(epochs):
            ...
            if mask_tensor_a_s is not None and mask_tensor_b_s is not None:
                ...
                mask_tensor = mask_tensor_a_s.squeeze((0, 1))
            elif mask_tensor_b_s is not None:
                mask_tensor = mask_tensor_b_s.squeeze((0, 1))
            else:
                mask_tensor = None
